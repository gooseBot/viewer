...
function addFilter() {
	...
	var uniqueDef = new esri.tasks.UniqueValueDefinition();
	uniqueDef.attributeField = "sessionUUID";
    var params = new esri.tasks.GenerateRendererParameters();
    params.classificationDefinition = uniqueDef;
	//am getting the first featureLayer to get the unique ids from, ID 0 should be the base scan which is what I want.
    var generateRenderer = new esri.tasks.GenerateRendererTask(map.getLayer(map.graphicsLayerIds[0]).url,{checkValueRange:"true"});
	var firstLayer = map.getLayer(map.graphicsLayerIds[0]);
	console.log(firstLayer.name);
    ...
    function applyRenderer(renderer) {
		...
		var data = renderer.toJson();
		for(var key in data.uniqueValueInfos) {
			var uniqueSessionID = data.uniqueValueInfos[key];
			console.log(uniqueSessionID.value);
			jsonArr.push({
				id: key,
				label: uniqueSessionID.value
			});
		}
		...
		{
			console.log('identifier: ' + treeDijit.getLabel(item)); 
			//apply a layer definition to see what happens
			for (var j = 0; j < map.graphicsLayerIds.length; j++) {
				var currentLayer = map.getLayer(map.graphicsLayerIds[j]);
				if (currentLayer.declaredClass === "esri.layers.FeatureLayer") {
					currentLayer.setDefinitionExpression("sessionUUID = '" + treeDijit.getLabel(item) + "'");
				}
			}
		});
