...
// needed to create custom LODs during utils.createMap
...
function createMap(webmapitem) {
	...
	//*************************************** Added by ERIC **********************************************************
	//combine the lods from the satellite basemap and the property layer so I can get extra zoom levels down to 70:1
	//  that way I can see more detail for the scan and target data.	
	var satellite = new esri.layers.ArcGISTiledMapServiceLayer("http://services.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer");
	satellite.on("load", addLods);
	var property = new esri.layers.ArcGISTiledMapServiceLayer("http://tiles.arcgis.com/tiles/25Iz4FI030a91YVh/arcgis/rest/services/property/MapServer");
	property.on("load", addLods);
	//****************************************************************************************************************

	//********************************************** Added by ERIC *****************************
	// once load count is 2 then my layers are added and we can continue the normal code	
	// populate an array with zoom levels and pass to createMap when both are loaded.
	function addLods(evt) {
	  customLods = customLods.concat(evt.layer.tileInfo.lods);
	  loadCount++;
	  if (loadCount === 2) {	
	  
		//ESRI Code now that my custom base map is loaded
		...
		mapDeferred.addCallback(function (response) {
			...
			//************************* added by ERIC **************************************
			//apply a layer definition to see what happens
			...
			//		currentLayer.setDefinitionExpression("sessionUUID = '1441105930'");
			//	}
			//}
			//******************************************************************************
	...
	}  //END of ERIC added function to load custom LODs
}
...
function initUI(response) {
    ...
    if (displayLeftPanel()) {
        ...
        if (configOptions.displaylegend === true) {
            addLegend(layerInfo);
			//************************************* Added by ERIC to support filtering ********************
			addFilter();              //add panel and button so I have a place to put the filter controls
			//*********************************************************************************************
        }
...
function toggleToolbarButtons(label) {
    var buttons = ['detailButton', 'editButton', 'legendButton', 'filterButton'];
...
//******************************************************* Added by ERIC to support filtering *******************************************
//Add the filter tool to the application - the filter will be 
//added to the left panel of the application. 
function addFilter() {

    var filterTb = new dijit.form.ToggleButton({
        showLabel: true,
        label: i18n.tools.filter.label,
        title: i18n.tools.filter.title,
        checked: true,
        iconClass: 'esriFilterIcon',
        id: 'filterButton'
    }, dojo.create('div'));

    dojo.byId('webmap-toolbar-left').appendChild(filterTb.domNode);

    dojo.connect(filterTb, 'onClick', function () {
        navigateStack('filterPanel');
    });
    var filterCp = new dijit.layout.ContentPane({
        title: i18n.tools.filter.title,
        selected: true,
        region: 'center',
        id: "filterPanel"
	...
	var outFields = ["CreationDate", "sessionUUID"];
	
	statDef1.statisticType = "min";
	statDef1.onStatisticField = "CreationDate";
	statDef1.outStatisticFieldName = "maxDate";	
	statDef2.statisticType = "count";	
	statDef2.onStatisticField = "CreationDate";
	statDef2.outStatisticFieldName = "countDate";	
	
	treeQuery.returnGeometry = false;
	treeQuery.groupByFieldsForStatistics = ["sessionUUID"] ;
	treeQuery.outFields = outFields;
	treeQuery.outStatistics = [ statDef1, statDef2];
	treeQueryTask.execute(treeQuery, populateTree);
	
    function populateTree(result) {
			
		var jsonArr = [];
		var sessions = [{id:"A", label:"Sessions", children:jsonArr}];		
		var data = result.toJson();          
		//sort the results by date descending
		data.features.sort(function(a, b) {
			return b.attributes.maxDate - a.attributes.maxDate;
		});
		//loop the results and added to the tree
		for(var key in data.features) {
			var feature = data.features[key];
			var featureDate = new Date(feature.attributes.maxDate);   //get the date of a feature
			// convert to just the month, day, year portion
			var dayOfattack = (featureDate.getMonth() + 1) + "-" + featureDate.getDate() + "-" + featureDate.getFullYear();	
			// create a date parent object for the dojo tree store
			var newObj = {id:dayOfattack, label:dayOfattack, children:[]};
			// get the index for an existing parent for this date
			var dateIndex = containsObject(newObj, jsonArr);
			// if no parent then add this one
			if (typeof dateIndex == 'undefined') {
				jsonArr.push(newObj);
				// find the index for where it went (must be a better way to do this)
				dateIndex = containsObject(newObj, jsonArr);
			}
			//now add the session info to the children children collection of the parent
			//var timeOfattack = featureDate.getHours() + ":" + featureDate.getMinutes() + ":" + featureDate.getSeconds();
			var timeOfattack = [featureDate.getHours().padLeft(),
								featureDate.getMinutes().padLeft(),
								featureDate.getSeconds().padLeft()].join(':');
			sessions[0].children[dateIndex].children.push({
				id: feature.attributes.sessionUUID,
				label: timeOfattack
			});
		}
		
		var objectStore = new dojo.store.Memory({data: sessions});
		...
		dojo.connect(treeDijit, 'onClick', function(item, node, evt)
		{
			//apply a layer definition to see what happens
			for (var j = 0; j < map.graphicsLayerIds.length; j++) {
				var currentLayer = map.getLayer(map.graphicsLayerIds[j]);
				if (currentLayer.declaredClass === "esri.layers.FeatureLayer") {
					currentLayer.setDefinitionExpression("sessionUUID = '" + item.id + "'");
					//make sure the layers are on after the filter is applied, 
					//  I turned them off in the webmap so all the data doesn't load
					//  initially with the map and slow 
					//currentLayer.setVisibleLayers([0]);
					currentLayer.refresh();
				}
			}
			
		});
		// find the filterpanel and add the tree to it.
		dojo.byId('filterPanel').appendChild(treeDijit.domNode);
		navigateStack('filterPanel');		
		treeDijit.startup();	
		
		// create a context menu so we can delete scans
		pMenu = new dijit.Menu({targetNodeIds:["contextMenu"]});
        pMenu.addChild(new dijit.MenuItem({
            label:"Delete",
            iconClass:"dijitEditorIcon dijitEditorIconDelete",
            onClick: function(){
                var selectedObject = treeDijit.get("selectedItems")[0];
				...
				for (var j = 0; j < map.graphicsLayerIds.length; j++) {
					...
					if (currentLayer.declaredClass === "esri.layers.FeatureLayer") {					
						esri.request({ 
							url: currentLayer.url + "/deleteFeatures", 
							handleAs: "json",
							content: {where:'sessionUUID='+ selectedObject.id, f:'json'},			
						}, {usePost:true});  						
					}
				}				
            }
